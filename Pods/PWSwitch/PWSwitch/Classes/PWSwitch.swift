...
open class PWSwitch: UIControl {
	...
	open var thumbWidth: CGFloat { // UI_APPEARANCE_SELECTOR
		...
		set {
			...
			self.thumbLayer.frame.size = CGSize(width: thumbWidth, height: self.thumbLayer.frame.height)
		}
	}
    ...
    override public init(frame: CGRect) {
        ...
        self._thumbWidth = _thumbDiameter
    ...
    fileprivate func getThumbOffRect() -> CGRect {
        return CGRect(x: (frame.height - thumbDiameter)/2.0, y: (frame.height - thumbDiameter)/2.0, width: _thumbWidth, height: thumbDiameter)
    }
    
    fileprivate func getThumbOffPushRect() -> CGRect {
        return CGRect(x: (frame.height - thumbDiameter)/2.0, y: (frame.height - thumbDiameter)/2.0, width: _thumbWidth + thumbDelta, height: thumbDiameter)
    }
    
    fileprivate func getThumbOnRect() -> CGRect {
        return CGRect(x: frame.width - thumbDiameter - ((frame.height - thumbDiameter)/2.0), y: (frame.height - thumbDiameter)/2.0, width: _thumbWidth, height: thumbDiameter)
    }
